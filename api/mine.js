export default async function handler(req, res) {
    ...
    const apiKey = process.env.GEMINI_API_KEY; // VOLVEMOS A USAR LA CLAVE DE GEMINI
    ...
    try {
        ...
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${apiKey}`, {
            ...
            body: JSON.stringify({
                ...
                generationConfig: { temperature: 0.3 } // Temperatura baja para que no invente
            })
        });

        const data = await response.json();
        
        // Si Gemini devuelve un error, lo mostramos
        if (data.error) {
